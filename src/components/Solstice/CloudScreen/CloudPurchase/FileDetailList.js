...
const FileDetailList = (props) => {
    ...
    React.useEffect(() => {
        if(fileList) {
            ...
            let temp = [...fileList?.filter(file => file.name.toLowerCase().search(searchStr.toLowerCase()) >= 0 
                && (viewFileList === 'recent' ? new Date().getTime() - new Date(file.created_at).getTime() < 24*60*60*1000*2 : true
                    ))] ;
    ...
    return (
        <Grid container>
                                ...
                                fileList ? (
                                    fileList.length ? 
                                        filterFileList.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((file, index) => {
                                            return(
                                                <TableRow key={index} onClick={() => handleSelectFile(page * rowsPerPage + index)} sx={{background : page * rowsPerPage + index === fileIndex && theme.palette.blue.B200}}>
                                                    <TableCell>
                                                        <Box sx={{display : 'flex', alignItems :'center', gap : '10px'}}>
                                                            ...
                                                            {
                                                                file.category === 'doc' && <img src={WordImage} width={40} />
                                                            }
                                                            {
                                                                file.category === 'docx' && <img src={WordImage} width={40} />
                                                            }
                                                            {file.name}
                                                        </Box>
                                                    </TableCell>
            ...
            <Grid item xs={match1190 ? 4 : 12} sx={{color : theme.palette.blue.B100, padding : match1190 ? '20px' : '5px', display : !fileList?.length && 'none', marginTop : '20px'}}>
                <FileInformation
