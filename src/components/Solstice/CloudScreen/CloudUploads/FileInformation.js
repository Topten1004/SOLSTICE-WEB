        ...
        <>
            ...
            <Box sx={{display : 'flex', flexDirection : 'column', justifyContent : 'center', alignItems : 'center', mt : '20px'}}>
                <Box className={classes.previewDiv}> 
                    {
                        fileInfo && (
                            fileInfo.category === 'video' ? <video src={fileInfo?.path} width={250} height={230} style={{background : theme.palette.blue.B300, borderRadius : '10px'}} controls/>
                            : fileInfo.category === 'image' ? <img src={fileInfo?.path} width={250} height={230} style={{background : theme.palette.blue.B300, borderRadius : '10px'}} />
                            : fileInfo.category === 'pdf' ? <PdfPreview
                                previewUrl={fileInfo.path}
                                width={250}
                                height={230}
                            /> 
                            : fileInfo.category === 'docx' ?  <Box sx={{display : 'flex', justifyContent : 'center', height:'230px', width:'250px'}}>
                                        <DocxPreview
                                        previewUrl={fileInfo.path}
                                        width={250}
                                        height={230}
                            ...
                            : fileInfo.category === 'doc' ?  <Box sx={{display : 'flex', justifyContent : 'center', height:'230px', width:'250px'}}>
                                        <DocPreview
                                            previewUrl={fileInfo.path}
                                            width={250}
                                            height={230}
                ...
                <Box sx={{width : '100%', color : 'gray', paddingLeft : '10px'}}>
                    Created : {new Date(fileInfo?.created_at).toLocaleDateString() || "Unknown"}
                </Box>
            </Box>
