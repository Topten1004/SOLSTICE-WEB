...
const ImageToCanvas = (props) => {
    ...
    return (
        <Box className={classes.root} sx={{height : (height + 6 ) + 'px !important', width : (width + 6) + 'px !important', }}>
            <canvas id={imageInfo?.imageId} width={width} height={height} style={{outline :(selected ? selectedColor : normalColor) + '1px solid', borderRadius : '20px'}}/>
            {
                !loaded && <Box className={classes.loadingDiv} 
                                sx={{height : ( height + 6 ) + 'px !important', width : (width+6) + 'px !important', border : '1px solid ' + normalColor, background : bgColor}}
                    >
                    <Loading type='oval' width={30} height={30} fill={theme.palette.green.G200} />
                    <Box>
                        Rendering...
                    </Box>
                </Box>
