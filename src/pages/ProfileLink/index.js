...
import { LoadingProfileLink, SellerProfileInfo, InitLinkReducer } from '../../redux/actions/link';

import Loading from 'react-loading-components' ;
import NotFound from '../../components/Common/NotFound';
import ProfileLinkScreen from './ProfileLinkScreen';
...
const ProfileLink = (props) => {
    ...
    React.useEffect(async () => {
        ...
        if( await SellerProfileInfo("https://solsapp.com" + location.pathname) ){
        ...
        <>
            {
                !loadingProfileLink ? <Box className={classes.loadingDiv}>
                    ...
                    <Box sx={{color : theme.palette.green.G200, fontSize : '30px', letterSpacing : '5px'}}>...Checking Link</Box>
                </Box> :
                (
